...
#include "MyTrigger/myTriggerStudies/interface/HLTHtLeadJetFilter.h"
...
// Constructor
HLTHtLeadJetFilter::HLTHtLeadJetFilter(const edm::ParameterSet & iConfig) : HLTFilter(iConfig),
  ...
  nOrs_      ( htLabels_.size() ) {  // number of settings to .OR.
    if (!( htLabels_.size() == minHt_.size() &&
        ...
        edm::LogError("HLTHtLeadJetFilter") << "inconsistent module configuration!";
    }
...
// Destructor
HLTHtLeadJetFilter::~HLTHtLeadJetFilter() {}

// Fill descriptions
void HLTHtLeadJetFilter::fillDescriptions(edm::ConfigurationDescriptions & descriptions) {
    std::vector<edm::InputTag> tmp1(1, edm::InputTag("hltHtLeadJetProducer"));
    ...
    descriptions.add("hltHtLeadJetFilter", desc);
}

// Make filter decision
bool HLTHtLeadJetFilter::hltFilter(edm::Event & iEvent, const edm::EventSetup & iSetup, trigger::TriggerFilterObjectWithRefs & filterproduct) const {
    bool accept = false;
	...
	filterproduct.addCollectionTag(htLabels_[i]);
      ...
      edm::Ref<reco::METCollection> mhtref(hht,0);
...
DEFINE_FWK_MODULE(HLTHtLeadJetFilter);
